...
export class NotificationService {
  ...
  public add(msg: string, type: NotificationType = NotificationType.error) {
    ...
    this.listado.push(n); 
    // Redundancia: Los errores también se muestran en consola if (type === NotificationType.error) { this.out.error(`NOTIFICATION: ${msg}`); } }
  }
  public remove(index: number) { if (index < 0 || index >= this.listado.length) { 
