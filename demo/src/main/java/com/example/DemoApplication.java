...
//		srv.getByProjection(ActorDTO.class).forEach(System.out::println);
	...
	CommandLineRunner lookup(CalculatorProxy client) {
		return args -> {
			System.err.println("Calculo remoto --> " + client.add(2, 3) + "\n" + "Calculo remoto --> "
					+ client.sub(2, 3) + "\n" + "Calculo remoto --> " + client.mul(2, 3) + "\n" + "Calculo remoto --> "
					+ client.div(2, 3) + "\n");
		};
	}
...
//			request.setOp1(2);
//			request.setOp2(3);
//			var response = (AddResponse) ws.marshalSendAndReceive("http://localhost:8090/ws/calculator", request,
